...
set "%~1dependencies= "
set "%~1dev_dependencies=input_string unset_all"
...
%=REQUIRED=%
%=REQUIRED=%
call :argparse._parse_opts %* || exit /b
...
call :argparse._parse_specs %* || exit /b 3
if defined _dry_run exit /b 0
if defined _new_name set "_name=!_new_name!"
call :argparse._parse_args %* || exit /b 4
call :argparse._capture_args || exit /b 4
exit /b 0
#+++

:argparse._parse_opts %*
::  -> _position + options
setlocal EnableDelayedExpansion
call :argparse._read_opt_spec || exit /b 2
set "_position=0"
set "_stop_on_extra=true"
call :argparse._parse_args %* || exit /b 3
set _actions=!_actions! "store_const:_position=!_position!"
call :argparse._capture_args || exit /b 3
exit /b 0
#+++

:argparse._parse_opt_spec
set "_position=0"
call :argparse._parse_specs ^
...
:argparse._read_opt_spec [return_prefix]
set "%~1_position=5"
set "%~1_spec_names="
set "%~1_spec_flags="
...
call :argparse._parse_spec_loop %* || (
    call :argparse._error read_spec "!errorlevel!" >&2
    exit /b 3
)
exit /b 0
#+++

:argparse._parse_spec_loop %*
...
for /l %%# in (1,1,20) do for /l %%# in (1,1,20) do (
    ...
    call :argparse._validate_spec || exit /b
...
call :argparse._parse_arg_loop %* || (
    set "_exit_code=!errorlevel!"
    call :argparse._error parse_args "!errorlevel!" >&2
    exit /b !_exit_code!
)
...
if defined _spec_required (
    set "_exit_code=5"
    call :argparse._error parse_args "!_exit_code!" >&2
    exit /b !_exit_code!
)
exit /b 0
#+++

:argparse._parse_arg_loop %*
...
for /l %%# in (1,1,32) do for /l %%# in (1,1,32) do (
    ...
    if defined _new_spec (
        ...
        if "!_action!" == "help" (
            call :argparse._generate_help _syntax
...
:argparse._capture_args $_actions
(
...
:argparse._generate_help <return_var>
setlocal EnableDelayedExpansion
set "_return_var=%~1"
set "_result="
for /f "tokens=1-6* delims=|" %%a in ("!_spec_flags!!_spec_names!") do (
    if %%a GEQ 0 (
        call :argparse._get_spec "%%a"
...
:argparse._error   <context> <exit_code>
setlocal EnableDelayedExpansion
set "_context=%~1"
set "_exit_code=%~2"
...
call :argparse --name "shop" --stop-nonopt ^
...
call :argparse._parse_opt_spec || (
    call %unittest% fail "Got error when parsing internal opt specs"
)
call :argparse._read_opt_spec _final || (
...
(
    ...
    if "%~1" == "read_opt_spec" (
        ...
        call :argparse._generate_help _spec_syntax
        echo Parse Args
        echo _position: !_position!
